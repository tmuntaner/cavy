...
describe 'admin item groups' do
  ...
  describe 'index' do
    ...
    it "should be able to go to index" do
      visit admin_item_groups_path
      current_path.should eq(admin_item_groups_path(locale: :en))
    end

    it "should not be able to visit index as a non user" do
      log_out
      visit admin_item_groups_path
      current_path.should_not eq(admin_item_groups_path(locale: :en))
    end
  ...
  describe 'new' do
    ...
    it "should be able to create a new item group" do
      visit admin_new_item_group_path(locale: :en)
      fill_in 'admin_item_group_title', with: 'testfoo'
      fill_in 'admin_item_group_param_string', with: 'test,bar'
      click_button 'Save'
      @group = Cavy::AdminItemGroup.find_by(title: 'testfoo')
  ...
  describe 'edit' do
    ...
    it "should be able to edit an item group" do
      ...
      fill_in 'admin_item_group_param_string', with: 'test,bar'
      click_button 'Save'
      @group = Cavy::AdminItemGroup.find(@group.id)
      @group.params.should eq(['test','bar'])
      @group.destroy
    end
    it "shouldn't be able to update a new item group without a title" do
      visit admin_edit_item_group_path(locale: :en, id: @group.id)
      fill_in 'admin_item_group_title', with: ''
      fill_in 'admin_item_group_param_string', with: 'test, bar'
      click_button 'Save'
      page.should have_content('error')
    end
  ...
  describe 'destroy' do
    ...
    it "should destroy a group item" do
      count = Cavy::AdminItemGroup.count
      visit admin_item_groups_path(locale: :en)
      click_link("delete-#{@group.id}")
      Cavy::AdminItemGroup.count.should eq(count-1)
    end
  end
end